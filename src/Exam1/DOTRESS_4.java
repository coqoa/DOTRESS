...
public class DOTRESS_4 extends Application {
	...
	private Node createObject(int x, int y, int w, int h, Color color) {
		...
		inGameRoot.getChildren().addAll(object);
		return object;
	}	// object를 생성해서 inGameRoot에 넣어준다

	private Node createPlayer(int x, int y, int w, int h, Color color) {
//		
//		Point2D playerCenterPoint2d = new Point2D(x+10, y-10);
//		Point2D	canonLength = new Point2D(x+20, y-20);
//		
	...
	private void MainPage() { 
		...
		player = createPlayer(randomNumberX, randomNumberY, 20, 20, Color.DODGERBLUE);
...
//		keys객체를 만드는 HashMap클래스에 쓰이는 메소드 getOrDefault = key가 존재하면 key값을 반환하고 존재하지않으면 디폴트값인 false를 반환
	...
	private void update() { // AnimationTimer로 1프레임마다 업데이트
		...
		if (isPressed(KeyCode.LEFT) && player.getTranslateX()+20 >= 0) { // LEFT키를 누르고 player객체의 x값이 0보다 크거나 같다면 movePlayerX의 매개변수로 -2를 입력
			movePlayerX(-2);	
			}
		if (isPressed(KeyCode.RIGHT) && player.getTranslateX() <= WIDTH) { // RIGHT키를 누르고 player객체의 x값+20(player객체크기)이 맵의 WIDTH보다 작거나 같다면 movePlayerX의 매개변수로 2를 입력
			movePlayerX(2);		
			}
		//
